...
then
    python main.py $console_only --method $mode --epochs 3000 
elif [[ $mode == 'pgd_both_metric' ]] || [[ $mode == 'pgd_both_robustness_metric' ]]
then
    python main.py $console_only --method $mode --epochs 300 --pgd_epsilon 0.005 --batch_size 12 
...
then
    python main.py $console_only --method $mode --gan_test_models "gan_both.pth" --batch_size 15 
elif [[ $mode == 'worker' ]]
then
